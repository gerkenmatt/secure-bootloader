...
class OTAHost:
    def __init__(self, device_name: str,
        ...
        self.slot_status_event = asyncio.Event() 
            ...
            try:
                ...
                if line:
                    if line.startswith("Active slot:"):
                        try:
                            self.active_slot = int(line.split(":")[1].strip())
                            if self.verbose:
                                print(GREEN + f"  DEV_LOG: Received status: Active slot is {self.active_slot}" + RESET)
                            self.slot_status_event.set() # Signal that we have received the status
                        except (ValueError, IndexError):
                            print(RED + f"  DEV_LOG: Could not parse slot status from: {line}" + RESET)
                    else:
                        log_msg = f"  DEV_LOG: {line}"
                        print(GREEN + log_msg + RESET)
                        self.device_logs.append(line)
                        self.log_event.set()
            except Exception as e:
                log_msg_raw = f"  DEV_LOG (raw, decode_err {e}): {data.hex()}"
    ...
    async def get_active_slot(self, timeout=5.0) -> int:
        """Sends the 'status' command and waits for the active slot response."""
        if not self.client or not self.client.is_connected:
            print(RED + "Not connected. Cannot get active slot." + RESET)
            return -1

        if self.verbose:
            print(YELLOW + "Querying for active slot..." + RESET)

        self.slot_status_event.clear()
        self.active_slot = -1

        if not await self.send_text_command("status"):
            print(RED + "Failed to send 'status' command." + RESET)
            return -1

        try:
            await asyncio.wait_for(self.slot_status_event.wait(), timeout=timeout)
            return self.active_slot
        except asyncio.TimeoutError:
            print(RED + "Timeout waiting for active slot status." + RESET)
            return -1
