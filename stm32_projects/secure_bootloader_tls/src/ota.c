...
void print_mbedtls_error(int ret_val);
                      ...
                      uint16_t      sig_len)
{
    ...
    // Disable caches if your platform requires it for data coherency with flash
    // or for security reasons. This will impact performance.
    // SCB_DisableICache();
    // SCB_DisableDCache();

    // Set custom allocators if MBEDTLS_PLATFORM_MEMORY is enabled
    // This typically only needs to be called once at application startup,
    // unless you need to change allocators dynamically.
    // If you call it here, ensure stdlib calloc/free are available or provide custom ones.
#if defined(MBEDTLS_PLATFORM_MEMORY) && !defined(MBEDTLS_PLATFORM_CALLOC_MACRO) && !defined(MBEDTLS_PLATFORM_FREE_MACRO)
    // Only call if not using compile-time macros for allocation
    // and if not already set globally.
    // Ensure 'calloc' and 'free' are standard library or your custom implementations.
    // ret = mbedtls_platform_set_calloc_free(calloc, free);
    // if (ret != 0) {
    //     log("mbedtls_platform_set_calloc_free failed\r\n");
    //     return false;
    // }
#endif

    // --- 1. Hash the firmware image ---
    log("Hashing firmware image ("); print_uint32_hex(data_len); log(" bytes)...\r\n");
    mbedtls_sha256_init(&sha_ctx);

    ret = mbedtls_sha256_starts(&sha_ctx, 0); // 0 for SHA-256, not 1 for SHA-224
    if (ret != 0) {
        log("mbedtls_sha256_starts_ret failed: "); print_mbedtls_error(ret);
    ...
    if (ret != 0) {
        log("mbedtls_sha256_update_ret failed: "); print_mbedtls_error(ret);
        mbedtls_sha256_free(&sha_ctx);
        return false;
    }
    ...
    if (ret != 0) {
        log("mbedtls_sha256_finish_ret failed: "); print_mbedtls_error(ret);
        mbedtls_sha256_free(&sha_ctx);
        return false;
    }
    mbedtls_sha256_free(&sha_ctx);

    // log("Firmware Hash: "); // Optional: print hash for debugging
    // for(int i=0; i<sizeof(hash); i++) { print_uint8_hex(hash[i]); log(" "); } log("\r\n");
    ...
    if (ret != 0) {
        log("mbedtls_pk_parse_public_key failed: "); print_mbedtls_error(ret);
        mbedtls_pk_free(&pk_ctx);
        return false;
    }

    // Optional: Check if the key is an EC key (good practice)
    if (!mbedtls_pk_can_do(&pk_ctx, MBEDTLS_PK_ECKEY)) { // Ensure MBEDTLS_PK_ECKEY is known
        log("Error: Parsed key is not an EC key.\r\n");
        mbedtls_pk_free(&pk_ctx);
        return false;
    }
    // log("Public key parsed successfully (ECKey).\r\n"); // Debug

    // --- 3. Verify the signature ---
    log("Verifying signature ("); print_uint16_hex(sig_len); log(" bytes)...\r\n");
    ret = mbedtls_pk_verify(&pk_ctx, MBEDTLS_MD_SHA256, hash, sizeof(hash), sig, sig_len);
    if (ret != 0) {
        log("!!! mbedtls_pk_verify FAILED: "); print_mbedtls_error(ret);
    } else {
    ...
    // --- Platform Re-initialization (if caches were disabled) ---
    // SCB_EnableICache();
    // SCB_EnableDCache();
...
// Helper function to print mbedTLS errors if MBEDTLS_ERROR_C is defined
void print_mbedtls_error(int ret_val) {
#if defined(MBEDTLS_ERROR_C)
    char err_buf[128];
    mbedtls_strerror(ret_val, err_buf, sizeof(err_buf));
    log(err_buf); log(" (-0x"); print_uint32_hex(-ret_val); log(")\r\n");
#else
    log("Error code -0x"); print_uint32_hex(-ret_val); log("\r\n");
#endif
}